import TherapyModel from "../models/Therapy.js";

class TherapyController {
    static createTherapy = async (req, res) => {
        const neworder = new TherapyModel(req.body);
        try {
    ...
    static getAllTherapy= async (req, res) => {
        try {
            const result = await TherapyModel.find()
            res.send(result)
        }
       catch (error) {
            console.log(error)
    ...
    static getAllTherapybyUser= async (req, res) => {
        try {
            const result = await TherapyModel.find({user:req.params.user}).sort({date:-1})
